        ...
        if len(matches) > 0:  # can play
            if self.isHuman:  # let the human select
                ...
                while choice < 1 or choice > len(matches):
                    choicestr = input("Enter the number corresponding to the card that you want to play: ")
...
def play_uno(numPlayers):
    ...
    pile = UnoPile(deck)
    # set up the players
    playerList = [UnoPlayer('AI-'+str(i),False,deck) for i in range(1,numPlayers)]
    ...
    print()
    playerList.append(UnoPlayer(name,True,deck))
...
def get_num_comp_players():
    '''get_num_comp_players() -> int
    gets the number of computer opponents'''
    # get the input
    while True:
        num_comp_players = input('How many computer opponents do you want: ')
        
        # make sure that the input is a nonnegative integer
        if num_comp_players.strip() == '':
            print('Please enter the number of computer opponents that you want.\n')
        elif not num_comp_players.isdigit():
            print(f'{num_comp_players} is not a nonnegative integer.\n')
        # make sure that there are enough opponents
        elif int(num_comp_players) < 1:
            print('You must have at least one opponent.\n')
        # make sure that there are not too many opponents
        elif int(num_comp_players) > 9:
            print('You have requested for too many opponents.\n')
        # valid input
        else:
            num_comp_players = int(num_comp_players)
            break
    print()
    return num_comp_players

num_comp_players = get_num_comp_players()
play_uno(num_comp_players + 1)